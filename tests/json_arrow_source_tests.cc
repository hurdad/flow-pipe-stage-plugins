...
TEST(JsonArrowSourceTest, ReadsCompressedJsonToArrowTable) {
  auto expected = BuildSampleTable();
  const std::string contents = R"({"id":1,"name":"alpha"})"
                               "\n"
                               R"({"id":2,"name":"beta"})"
                               "\n";
  bool ran_case = false;
  ...
  for (const auto& test_case : CompressionCases()) {
    ...
    auto path = MakeTempPath(std::string("input_") + test_case.name + ".json");
