...
// KafkaSource
// ============================================================
class KafkaSource final : public ISourceStage, public ConfigurableStage {
 public:
  ...
  bool configure(const google::protobuf::Struct& config) override {
    ...
    if (!ProtobufConfigParser<KafkaSourceConfig>::Parse(config, &cfg, &error)) {
      FP_LOG_ERROR("kafka_source invalid config: " + error);
      return false;
    }

    if (cfg.brokers().empty() || cfg.topic().empty() || cfg.group_id().empty()) {
 ...
 private:
  bool InitializeConsumer() {
    ...
    if (rd_kafka_conf_set(conf, "bootstrap.servers", config_.brokers().c_str(), errstr,
      ...
      FP_LOG_ERROR("kafka_source invalid brokers config: " + std::string(errstr));
      rd_kafka_conf_destroy(conf);
      return false;
    }

    if (rd_kafka_conf_set(conf, "group.id", config_.group_id().c_str(), errstr, sizeof(errstr)) !=
        RD_KAFKA_CONF_OK) {
      FP_LOG_ERROR("kafka_source invalid group.id config: " + std::string(errstr));
      rd_kafka_conf_destroy(conf);
      return false;
    }
    ...
    if (!config_.client_id().empty()) {
      if (rd_kafka_conf_set(conf, "client.id", config_.client_id().c_str(), errstr,
                            sizeof(errstr)) != RD_KAFKA_CONF_OK) {
        FP_LOG_ERROR("kafka_source invalid client.id config: " + std::string(errstr));
        rd_kafka_conf_destroy(conf);
        return false;
      }
    }
    ...
    if (!config_.auto_offset_reset().empty()) {
      if (rd_kafka_conf_set(conf, "auto.offset.reset", config_.auto_offset_reset().c_str(), errstr,
                            sizeof(errstr)) != RD_KAFKA_CONF_OK) {
        FP_LOG_ERROR("kafka_source invalid auto.offset.reset config: " + std::string(errstr));
        rd_kafka_conf_destroy(conf);
        return false;
      }
    }
    ...
    for (const auto& entry : config_.consumer_config()) {
      if (rd_kafka_conf_set(conf, entry.first.c_str(), entry.second.c_str(), errstr,
                            sizeof(errstr)) != RD_KAFKA_CONF_OK) {
        FP_LOG_ERROR("kafka_source invalid consumer config " + entry.first + ": " +
                     std::string(errstr));
        rd_kafka_conf_destroy(conf);
        return false;
      }
    ...
    if (!consumer_) {
      FP_LOG_ERROR("kafka_source failed to create consumer: " + std::string(errstr));
      rd_kafka_conf_destroy(conf);
      return false;
    }
    ...
    if (config_.has_partition() && config_.partition() >= 0) {
      ...
      rd_kafka_topic_partition_list_add(partitions, config_.topic().c_str(), config_.partition());
      ...
      if (err != RD_KAFKA_RESP_ERR_NO_ERROR) {
        FP_LOG_ERROR("kafka_source failed to subscribe: " + std::string(rd_kafka_err2str(err)));
        ShutdownConsumer();
        return false;
      }
    }
